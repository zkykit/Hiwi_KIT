...
def viz(img, flo,path_base_img):
    ...
    '''
    cv2.imwrite('/media/zky/T7/task3/RAFT/demo_file_save/test'+'/'+f'{i}.png',img_flo[:, :, ::-1]) #
...
def demo(args,path_base_img=None):
    ...
    with torch.no_grad():#used for not calculate gradient
        images = glob.glob(os.path.join("/media/zky/T7/task3/RAFT/frame_small/L00_frame/2022_03_10_S01_SC00_T01_SE001_L00",'*.png'))#origin images path



        foto_save_name = '/media/zky/T7/task3/RAFT/demo_file_save/test'#flow image saved path
...
if __name__ == '__main__':
    ...
    path_base = '/media/zky/T7/task3/RAFT/frame_small/L00_frame'
